...
class SobelEdge(nn.Module):
    ...
    def forward(self, x):
        ...
        edge_x = F.conv2d(gray, self.sobel_kernel_x, padding=1)
        edge_y = F.conv2d(gray, self.sobel_kernel_y, padding=1)
...
class ImageEnhancementModule(nn.Module):
    def __init__(self):
        ...
        self.gaussian_blur = nn.Conv2d(
            3, 3, kernel_size=5, stride=1, padding=2,
            groups=3, bias=False
        )
        self._init_gaussian_kernel(5, 1.5)

        # 可学习参数
        self.contrast = nn.Parameter(torch.tensor([1.0]))
        self.brightness = nn.Parameter(torch.tensor([0.0]))

    def _init_gaussian_kernel(self, kernel_size, sigma):
        ...
        self.gaussian_blur.weight.data = kernel
    ...
    def forward(self, x):
        ...
        # 对比度亮度调整
        x = x * torch.clamp(self.contrast, 0.5, 1.5) + torch.clamp(self.brightness, -0.2, 0.2)
        return x + identity * 0.2
@MODELS.register_module()
class STDCContextPathNet2(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None):
        super().__init__(init_cfg=init_cfg)
        self.enhance = ImageEnhancementModule()
