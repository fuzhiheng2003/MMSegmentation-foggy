...
class Attention(torch.nn.Module):
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        token_head = H
        qq = self.to_q(x).reshape(B, self.num_heads, self.key_dim, token_head, H * W//token_head).permute(0, 1, 3, 4, 2)
        kk = self.to_k(x).reshape(B, self.num_heads, self.key_dim, token_head, H * W//token_head).permute(0, 1, 3, 2, 4)
